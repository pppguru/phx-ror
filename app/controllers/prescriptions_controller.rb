class PrescriptionsController < ApplicationController
    ...
    def getRxValidationInfo

      if (prescription = Prescription.find_by_rx_number( params[:rx_number] )).nil?
        render :json => {:success => "false", :msg => "Prescription not found"}
        return
        # format.html { redirect_to rxVerification_url, notice: 'Prescription not found.' }
      else
        dispense = Dispense.find_by rx_number: params[:rx_number], fill_number: prescription.last_fill_number

        if dispense
          item = Item.find dispense.item_id

          if item
            render :json => {:success => "true", :item => item}
            return
          else
            render :json => {:success => "false", :msg => "Item not found"}
            return
          end
        else
          render :json => {:success => "false", :msg => "Dispense not found"}
          return
        end

      end

      render :json => {:success => "false", :msg => "Unknown Error"}
    end
