...
class ClaimsController < ApplicationController
    ...
    def search
      dbContext = Claim.joins(:customer, :plan).select('claims.id, rx_number, fill_number, claims.plan_id_code, date_filled, status, total_submitted, cost_submitted, last_name, insurance_plan_name')
      ...
      if params[:by_name] && params[:by_name] == '1'

        if params[:customer_name] && params[:customer_name] != ''
          dbContext = dbContext.where("last_name like  '#{params[:customer_name]}%'")
        end

        if params[:plan_name] && params[:plan_name] != ''
          dbContext = dbContext.where("insurance_plan_name like  '#{params[:plan_name]}%'")
        end
